...
class DreameMowerDevice:
    ...
    def _handle_properties(self, properties) -> bool:
        ...
        if not self._ready:
            self.capability.refresh(
                json.loads(
                    zlib.decompress(
                        base64.b64decode(DREAME_MODEL_CAPABILITIES), zlib.MAX_WBITS | 32
                    )
                )
            )
    ...
    def update(self, force_request_properties=False) -> None:
        ...
        if self.status.active:
            ...
            properties.extend(
                [DreameMowerProperty.CLEANED_AREA, DreameMowerProperty.CLEANING_TIME]
            )
    ...
    def clean_zone(
        ...
        if not isinstance(zones, list) or not zones:
            raise InvalidActionException("Invalid zone coordinates: %s", zones)
        ...
        for zone in zones:
            if not isinstance(zone, list) or len(zone) != 4:
                raise InvalidActionException("Invalid zone coordinates: %s", zone)
    ...
    def clean_spot(
        ...
        if not isinstance(points, list) or not points:
            raise InvalidActionException("Invalid point coordinates: %s", points)
        ...
        if not self.capability.cruising:
            ...
            if self.status.current_map and self.status.current_map.robot_position:
                ...
                if abs(x - position.x) <= size and abs(y - position.y) <= size:
                    raise InvalidActionException(
                        "Robot is already on selected coordinate"
                    )
    ...
    def follow_path(self, points: list[int] | list[list[int]]) -> dict[str, Any] | None:
        ...
        if self.status.stream_status != DreameMowerStreamStatus.IDLE:
            raise InvalidActionException(
                "Follow path only works with live camera streaming"
            )
        ...
        if self.capability.ai_detection:
            ...
            try:
                if isinstance(ai_value, int):
                    ...
                    result = self.set_ai_detection(
                        {DreameMowerStrAIProperty[prop.name].value: bool(value)}
                    )
    ...
    def set_camera_light_brightness(self, brightness: int) -> dict[str, Any] | None:
        if self.capability.auto_switch_settings:
            ...
            if result is None or result.get("code") != 0:
                self._update_property(
                    DreameMowerProperty.CAMERA_LIGHT_BRIGHTNESS, str(current_value)
                )
            return result
...
class DreameMowerDeviceStatus:
    ...
    def returning(self) -> bool:
        ...
        return bool(
            self._device_connected and (self.status is DreameMowerStatus.BACK_HOME)
        )
